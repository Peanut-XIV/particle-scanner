...
def get_now():
    return dt.datetime.now(tz=dt.timezone(dt.timedelta(hours=2)))
...
class Scanner(object):
    def __init__(self, controller):
        ...
        self.X_STEP = self.controller.horizontal_stack_offset[0]  # µm
        self.Y_STEP = self.controller.horizontal_stack_offset[1]  # µm
        self.image_width = 2100
        self.image_height = 1760
        ...
        self.summary = {
            ...
            'exposure (µs)': self.config.exposure_time\
                    if self.multi_exp is None else self.multi_exp,
    ...
    def lowest_corner(self) -> int:
        ...
        brz = br[2]
        frz = flz - brz + blz
        mini = max(0, min(blz, brz, flz, frz))
        return mini
    ...
    def multi_scan(self):
        ...
        nonempty_dir = len(list(self.controller.save_dir.iterdir())) > 0
        if self.controller.save_dir.exists() and nonempty_dir:
            if self.controller.do_overwrite:
                utils.remove_folder(self.controller.save_dir)
                os.makedirs(self.controller.save_dir)
            else:
                parent = self.controller.save_dir.parent
                self.controller.save_dir = utils.make_unique_subdir(parent)
        ...
        if self.auto_f_stack:
            ...
            arguments = (self.queue,
                         error_logs,
                         self.multi_exp,
                         self.controller.remove_raw)
            self.parallel_process =\
                    mp.Process(target=helicon_stack.parallel_stack,
                               args=arguments)
        ...
        for n in range(len(self.config.scans)):
            ...
            self.summary['scan_dates'].append(get_now())
            self.scan(scan_dir)
        self.summary['scan_dates'].append(get_now())
        ...
        if self.auto_quit:
            ...
            self.controller.interrupt_flag =\
                    bool(self.controller.quit_requested)
    ...
    def take_stack(self, dx, dy, scan_dir):
        ...
        xy_folder = Path(scan_dir)\
                .joinpath(f"X{self.stage.x//10:05d}_Y{self.stage.y//10:05d}")
        ...
        for _ in range(self.stack_count):
            for exp in exp_values:
                ...
                name_str = f"X{x//10:05d}_Y{y//10:05d}_Z{z//10:05d}.jpg"
                save_path = save_path.joinpath(name_str)
                skio.imsave(str(save_path),
                            img[..., ::-1],
    ...
    def make_scan_summary(self):
        ...
        with open(summary_path, mode='x', encoding='UTF-8') as summary:
            if self.controller.interrupt_flag:
                summary.write('///////////'
                              'THE SCANS WERE INTERRUPTED BEFORE FINISHING!!!'
                              '///////////\n\n')
            ...
            for n in range(len(deltas)):
                ...
                summary.write(
                    f"scan{n + 1} started at {dates[n]}, "
                    f"lasted {h}h {m}min {s}s and took :\n"
            ...
            summary.write(f'Overall, the task ended at {dates[-1]} and lasted '
                          f'{h}h {m}min and {s}s.\n')
